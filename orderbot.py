...
def into_db(message):
	...
	order_list = [new_order_id, order_fio, order_info, order_adress, order_phone]
	connect = sqlite3.connect('orders.db')
	cursor = connect.cursor()
	cursor.execute("""CREATE TABLE IF NOT EXISTS orders(
		order_id INTEGER,
	...
	""")
	connect.commit()
	cursor.execute("INSERT INTO orders VALUES(?,?,?,?,?);", order_list)
	connect.commit()
